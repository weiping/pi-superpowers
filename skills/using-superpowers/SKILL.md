...
当 skills 引用以下工具时，在 Pi 中使用对应替代：

| Skills 中的工具 | Pi 中的替代方案 |
|---------------|--------------|
| `Skill` 工具 | 使用 `read` 工具读取技能文件路径，或在编辑器输入 `/skill:<name>` 命令加载 |
| `TodoWrite` | 使用 `write`/`edit` 工具操作项目根目录的 `TODO.md`，用 Markdown 复选框记录任务 |
| `Task`（子代理）| Pi 暂无内置子代理。使用顺序执行模式：在当前对话中逐任务实现，每任务完成后审查再继续 |
| `Read` | `read` 工具（同名，直接使用） |
| `Write` | `write` 工具（同名，直接使用） |
| `Edit` | `edit` 工具（同名，直接使用） |
| `Bash` | `bash` 工具（同名，直接使用） |

**Skills 位置**：所有 superpowers skills 已加载，直接用 `/skill:<name>` 命令调用。
